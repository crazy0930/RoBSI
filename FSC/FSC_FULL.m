function [solution,rmse,cor1_new,cor2_new]=FSC_FULL(cor1_low,cor2_low,cor1_up,cor2_up,change_form,error_t)
...
[M2,N2]=size(cor1_up);
cor1_new=zeros(M2,N2);
cor2_new=zeros(M2,N2);
...
for i=1:1:iterations
    while(1)
        ...
        cor11=cor1_low(a,1:2);
        cor22=cor2_low(a,1:2);
    ...
    solution=[parameters(1),parameters(2),parameters(5);
     ...
     match1_xy=[cor1_up(:,1:2)';ones(1,M2)];
    %for perspective
    if(strcmp(change_form,'perspective'))
        ...
        match2_xy=[cor2_up(:,1:2)';ones(1,M2)];
    ...
    if(consensus_num>most_consensus_number)
        ...
        cor1_new=cor1_up(index_in,:);
        cor2_new=cor2_up(index_in,:);
    end
end

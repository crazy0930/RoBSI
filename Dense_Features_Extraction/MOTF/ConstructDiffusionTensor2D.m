...
if((Options.eigenmode==0)||(Options.eigenmode==1)) % Weickert line shaped 
    ...
    lambda1 = Options.alpha + (1 - Options.alpha)*exp(-Options.C./di.^(2*Options.m)); 
    lambda2 = Options.alpha; 
elseif(Options.eigenmode==2) % EED：edge-enhancing diffusion 
    gradA=mean(gradA,3);
    lambda2 =1 - exp(-3.31488./(double(gradA./Options.lambda_e^2)).^4);
%     lambda2(lambda2>1e-10)=1;
     lambda2(gradA<1e-15)=1;
    lambda1 = 1;
elseif(Options.eigenmode==3) % CED：coherence-enhancing diffusion
   di=(mu1-mu2); di((di<1e-15)&(di>-1e-15))=1e-15;
    lambda1 = Options.alpha + (1 - Options.alpha)*exp(-Options.C./di.^(2*Options.m)); 
    lambda2 = Options.alpha; 
elseif(Options.eigenmode==4) % Hybrid Diffusion with Continous Switch: 具有连续开关的混合扩散
    gradA=mean(gradA,3);
    lambdae2 = 1 - exp(-3.31488./(double(gradA./Options.lambda_e^2)).^4);
